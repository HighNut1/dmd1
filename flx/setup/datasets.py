...
# 注意这里的导入：除了原有的，我们必须加上 Nist14Loader
...
NIST14_BINARIZATION = LazilyAllocatedBinarizer(4.0)  # <-- 为 NIST14 准备的二值化器
...
def get_nist_sd4(root_dir: str) -> TransformedImageLoader:
    ...
    assert loader.ids.num_subjects == NUM_SUBJECTS
    # 我也帮你把这个注释保留了，以防 SD4 数据集也有轻微缺失
    # assert len(loader.ids) == NUM_SUBJECTS * 2
    return Dataset(loader, loader.ids)


# ---------- 关键点：这个函数必须存在 ----------
def get_nist14(root_dir: str) -> Dataset:
    NUM_SUBJECTS = 2000
    loader = TransformedImageLoader(
        images=Nist14Loader(root_dir),
        poses=None,
        transforms=[NIST14_BINARIZATION],
    )
    assert loader.ids.num_subjects == NUM_SUBJECTS
    
    # 修复：真正注释掉引起 NIST14 加载报错的这一行
    # assert len(loader.ids) == NUM_SUBJECTS * 2
